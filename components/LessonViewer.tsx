    ...
    <div className="fixed inset-0 z-50 bg-tp-navy/95 backdrop-blur-xl flex flex-col animate-fadeIn">
      ...
      <header className="flex items-center justify-between px-8 py-4 bg-tp-purple border-b border-white/10">
        <div className="flex items-center">
            ...
            <div>
                <h2 className="text-white font-black text-lg tracking-tight uppercase">{lesson.title}</h2>
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">{lesson.type} • {lesson.level}</p>
            </div>
        </div>
        <button 
            onClick={onClose}
            ...
            <div className="w-full h-full flex flex-col">
                ...
                {!embeddedUrl.includes('youtube.com/embed') && (
                    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur-md px-6 py-3 rounded-2xl shadow-2xl border border-gray-100 flex items-center space-x-4">
                        <p className="text-xs font-bold text-tp-purple">Difficulty viewing?</p>
                        <a 
                            href={lesson.link} 
                            target="_blank" 
                            rel="noopener noreferrer"
                            className="text-xs font-black text-tp-red hover:underline uppercase tracking-widest"
                        >
                            Open in New Tab
                        </a>
                    </div>
      ...
      <footer className="px-8 py-6 bg-tp-navy border-t border-white/5 flex justify-center">
        <button 
            onClick={onComplete}
            className="bg-tp-red text-white font-black py-4 px-16 rounded-2xl hover:bg-red-700 hover:scale-105 active:scale-95 transition-all shadow-2xl shadow-tp-red/40 uppercase tracking-[0.2em] text-sm flex items-center"
        >
