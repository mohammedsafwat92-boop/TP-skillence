...
const NavItem: React.FC<{ view: View, currentView: View, onNavigate: (view: View) => void, children: React.ReactNode }> = ({ view, currentView, onNavigate, children }) => {
  ...
  return (
    <li className="mb-1.5 px-3">
      <button onClick={() => onNavigate(view)} className={`flex items-center w-full py-3 md:py-2.5 px-4 rounded-xl transition-all duration-200 text-left min-h-[48px] ${isActive ? 'bg-tp-red text-white font-semibold shadow-lg shadow-tp-red/20 translate-x-1' : 'bg-transparent text-gray-400 hover:bg-white/10 hover:text-white'}`}>
        {children}
      </button>
    </li>
  );
    ...
    <div className={`fixed inset-y-0 left-0 w-72 bg-tp-purple text-white flex flex-col h-full shadow-2xl z-50 transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:z-20 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <div className="px-6 py-6">
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
            <label className="text-[10px] text-gray-400 uppercase font-bold tracking-widest mb-2 block">Impersonate Profile</label>
            <select 
      ...
      <nav className="flex-1 py-4 overflow-y-auto custom-scrollbar">
        ...
        <div className="mt-8 mb-4 px-6"><h2 className="text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em]">Learning Tracks</h2></div>
        <ul>
        ...
        <div className="mt-8 mb-4 px-6"><h2 className="text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em]">Performance</h2></div>
        <ul>
            {quizzes.map(quiz => (
      ...
      <div className="p-6 mt-auto">
          <div className="bg-white/5 border border-white/10 rounded-2xl p-4 text-center">
              <p className="text-[10px] text-gray-400 font-bold uppercase">Internal Training Program</p>
